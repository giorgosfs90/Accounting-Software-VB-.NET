...
Public Class Karteles_pelatwn
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        Try
            ...
            query = "select imerominia AS 'ΗΜΕΡΟΜΗΝΙΑ', arithmos as 'ΑΡ.ΠΑΡ', perigrafi as 'ΠΕΡΙΓΡΑΦΗ',poso as 'ΠΟΣΟ',katharo_poso as 'ΚΑΘ.ΠΟΣΟ',fpa as 'ΦΠΑ' from logistiko.kiniseis where str_to_date(imerominia ,'%d/%m/%Y') between  str_to_date('" & TextBox2.Text & "','%d/%m/%Y') and str_to_date('" & TextBox3.Text & "','%d/%m/%Y') and onoma_pelati like '%" & TextBox1.Text & "%'  "
            query2 = "select sum(poso) from kiniseis where str_to_date(imerominia ,'%d/%m/%Y') between  str_to_date('" & TextBox2.Text & "','%d/%m/%Y') and str_to_date('" & TextBox3.Text & "','%d/%m/%Y') and onoma_pelati like '%" & TextBox1.Text & "%'"
    ...
    Sub TextBox1_TextChanged(sender As Object, e As EventArgs) Handles TextBox1.TextChanged
        ...
        Try
            connection.Open()
            Dim query As String
            query = "select id as 'ΚΩΔΙΚΟΣ', epwnumia as ' ΕΠΩΝΥΜΙΑ' from logistiko.pelates where epwnumia like '%" & TextBox1.Text & "%' "
            command = New MySqlCommand(query, connection)
            data.SelectCommand = command
            data.Fill(data_table)
            binding_source.DataSource = data_table
